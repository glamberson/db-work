...
class MatchingDetailWindow(QMainWindow):
    ...
    def bulk_accept_mrl(self):
        ...
        if not selected_matches:
            QMessageBox.information(self, "No Matches", "No matches meet the threshold criteria.")
            return

        reply = QMessageBox.question(
            self,
            "Confirm Bulk Accept",
            f"Are you sure you want to bulk accept {len(selected_matches)} matches with a threshold of {threshold}%?",
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                QMessageBox.information(self, "Bulk Accept", f"Successfully bulk accepted {len(selected_matches)} matches.")
    ...
    def bulk_accept_both(self):
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Bulk Accept",
            f"Are you sure you want to bulk accept {len(selected_matches)} matches with fulfillment items at a threshold of {threshold}%?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No
        )
        ...
        if reply == QMessageBox.Yes:
            ...
            fulfillment_item_ids = [
                match.fulfillment_record.fulfillment_item_id
                for match in selected_matches
                if hasattr(match, 'fulfillment_record') and match.fulfillment_record is not None
            ]
            match_scores = [match.score for match in selected_matches]
            match_grades = [self.get_match_grade(match.score) for match in selected_matches]
            matched_fields = [list(match.field_scores.keys()) for match in selected_matches]
            ...
            try:
                ...
                QMessageBox.information(self, "Bulk Accept", f"Successfully bulk accepted {len(selected_matches)} matches with fulfillment items.")
                self.update_match_display()
            except Exception as e:
    ...
    def create_main_view_tab(self):
        ...
        # Add toggle button
        self.view_toggle_button = QPushButton("Switch to Summary View")
    ...
    def toggle_main_view(self):
        is_summary = self.view_toggle_button.text() == "Switch to Summary View"
        self.view_toggle_button.setText("Switch to Detailed View" if is_summary else "Switch to Summary View")

        for i, match_widget in enumerate(self.match_widgets):
            self.update_match_widget_view(match_widget, is_summary)
    ...
    def create_match_widget(self, match: Match, index: int):
        ...
        # Create compact and expanded widgets
        compact_widget = self.create_compact_view(match)
        compact_widget.setObjectName("compact_view")
        expanded_widget = self.create_expanded_view(match)
        expanded_widget.setObjectName("expanded_view")

        # Initially, show compact view and hide expanded view
        ...
        toggle_button.setObjectName("expand_button")
        toggle_button.clicked.connect(lambda checked, ew=expanded_widget, btn=toggle_button: self.toggle_expanded_view(ew, btn))
    ...
    def toggle_main_view(self):
        is_detailed = self.view_toggle_button.text() == "Switch to Summary View"
        self.view_toggle_button.setText("Switch to Detailed View" if is_detailed else "Switch to Summary View")
        
        for widget in self.match_widgets:
            self.update_match_widget_view(widget, not is_detailed)

    def update_match_widget_view(self, match_widget, is_summary: bool):
        # Find compact and expanded widgets within the match widget
        compact_widget = match_widget.findChild(QWidget, "compact_view")
        expanded_widget = match_widget.findChild(QWidget, "expanded_view")
        toggle_button = match_widget.findChild(QPushButton, "expand_button")
